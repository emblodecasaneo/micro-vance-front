<template>
  <div>
    ...
    <div class="flex items-center mb-6">
      ...
      <h1 class="text-sm font-medium tracking-widest text-gray-700 ml-2">Dépôt sur compte d'épargne</h1>
    </div>
      ...
      <div class="md:col-span-1">
        <div class="bg-white shadow rounded-lg overflow-hidden">
          <div class="p-4 bg-green-50 border-b border-gray-200">
            <h3 class="text-xs font-semibold text-green-700">Informations du compte</h3>
          </div>
          <div class="p-4 space-y-3">
            <div class="grid grid-cols-2 gap-2">
              ...
              <span class="text-xs font-bold text-green-600">{{ formatCurrency(account.balance) }}</span>
      ...
      <div class="md:col-span-2">
        <div class="bg-white shadow rounded-lg overflow-hidden">
          <div class="p-4 bg-primary bg-opacity-10 border-b border-gray-200">
            <h3 class="text-xs font-semibold text-primary">Effectuer un dépôt</h3>
          </div>
          <div class="p-6">
            <form @submit.prevent="handleDeposit">
              <!-- Montant du dépôt -->
              <div class="mb-4">
                <label for="amount" class="block text-xs font-normal text-gray-500 mb-1">Montant du dépôt <span class="text-red-500">*</span></label>
                ...
                <p v-else-if="depositData.amount > 0" class="mt-1 text-xs text-green-600">
                  Nouveau solde après dépôt: {{ formatCurrency(parseFloat(account.balance) + parseFloat(depositData.amount)) }}
                </p>
              </div>
              
              <!-- Mode de paiement -->
              <div class="mb-4">
                <label for="payment_method" class="block text-xs font-normal text-gray-500 mb-1">Mode de paiement <span class="text-red-500">*</span></label>
                <select 
                  id="payment_method"
                  v-model="depositData.payment_method"
                  required
                  disabled
                  class="block text-xs w-full rounded-md h-[34px] px-3 border border-gray-300"
                  :disabled="processing"
                >
              ...
              <!-- Description -->
              <div class="mb-4">
                <label for="description" class="block text-xs font-normal text-gray-500 mb-1">Description</label>
                <textarea 
                  id="description"
                  v-model="depositData.description"
                  rows="1"
                  disabled
...
const depositData = ref({
  amount: '5000',
  description: 'Reception des fonds en espèces par le client',
  payment_method: 'cash',
...
const loadAccountDetails = async () => {
  ...
  try {
    ...
    if (account.value && account.value.status !== 'active') {
      errorMessage.value = 'Ce compte n\'est pas actif. Les dépôts ne sont autorisés que sur les comptes actifs.';
...
const handleDeposit = async () => {
  ...
  // Validation simple
  if (!depositData.value.amount || parseFloat(depositData.value.amount) <= 0) {
    formErrors.value.amount = 'Veuillez entrer un montant valide';
    return;
  }
  
  if (!depositData.value.payment_method) {
    formErrors.value.payment_method = 'Veuillez sélectionner un mode de paiement';
    return;
  }
  
  if (!depositData.value.transaction_date) {
  ...
  try {
    const result = await store.dispatch('savings/depositToAccount', {
      accountId,
      depositData: depositData.value
    });
    
    if (result.success) {
      toastStore.success('Dépôt effectué avec succès');
      ...
      if (result.errors) {
        ...
        toastStore.error('Erreur lors du dépôt');
      }
    }
  } catch (err) {
    console.error('Erreur lors du dépôt:', err);
    toastStore.error('Une erreur est survenue lors du dépôt');
  } finally {
...
const isFormValid = computed(() => {
  ...
  if (depositData.value.payment_method !== 'cash' && !depositData.value.reference_number) {
    return false;
  }
